...
# 兼容 NoMaD / 非NoMaD 模型，不卸载任何库
try:
    from diffusers.schedulers.scheduling_ddpm import DDPMScheduler
except ImportError:
    # 无diffusers时创建虚拟类，DA3/GNM/ViNT 正常运行
    class DDPMScheduler:
        pass
import wandb
import os
...
def train_eval_loop(
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        ...
        if should_eval:
            ...
            print(f"Skipping eval for epoch {epoch}; eval_freq={eval_freq}")

        checkpoint = {
            "epoch": epoch,
            "model": model,
            "optimizer": optimizer,
            "avg_total_test_loss": np.mean(avg_total_test_loss) if avg_total_test_loss else None,
            "scheduler": scheduler
        }
        ...
        if scheduler is not None:
            ...
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                if avg_total_test_loss:
                    scheduler.step(np.mean(avg_total_test_loss))
            else:
                scheduler.step()
        wandb_log_data = {"lr": optimizer.param_groups[0]["lr"]}
        if avg_total_test_loss:
            wandb_log_data["avg_total_test_loss"] = np.mean(avg_total_test_loss)
