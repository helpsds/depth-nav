...
# 放在 from __future__ import annotations 下面
try:
    from diffusers.training_utils import EMAModel
except ImportError:
    # 无diffusers时定义虚拟类，不影响DA3/GNM/ViNT训练
    class EMAModel:
        pass
import wandb
import os
...
def _is_da3_model(model: nn.Module) -> bool:
    base_model = _unwrap_model(model)
    model_class = base_model.__class__.__name__
    model_module = base_model.__class__.__module__
    return (
        model_class in {"DepthAnything3Net", "NestedDepthAnything3Net"}
        or model_module.endswith("depth_anything_3.model.da3")
        or (
            hasattr(base_model, "backbone")
            and hasattr(base_model, "head")
            and hasattr(base_model, "depth_patch_embed")
        )
    )


def _is_da3_dataset(dataset) -> bool:
    if hasattr(dataset, "da3_process_res"):
        return True
    if isinstance(dataset, ConcatDataset):
        return any(_is_da3_dataset(child) for child in dataset.datasets)
    return False


def _forward_da3_nav_model(model: nn.Module, obs_image: torch.Tensor, goal_image: torch.Tensor):
    frames = list(torch.split(obs_image, 3, dim=1))
    history_imgs = torch.stack(frames[:5], dim=1)  # [B,5,3,H,W]
    obs_frame = frames[5]  # [B,3,H,W]
    return model(
        obs_img=obs_frame,
        goal_img=goal_image,
...
def _compute_losses(
    ...
    def action_reduce(unreduced_loss: torch.Tensor):
        ...
        action_mask_float = action_mask.to(torch.float32)
        return (unreduced_loss * action_mask_float).mean() / (action_mask_float.mean() + 1e-2)
...
def _log_data(
    ...
    if num_images_log > 0 and image_log_freq != 0 and i % image_log_freq == 0:
        visualize_dist_pred(
            to_numpy(obs_image),
            to_numpy(goal_image),
            to_numpy(dist_pred),
...
def train(
    ...
    """
    model.train()
    # Check through wrappers such as ConcatDataset.
    is_da3_dataset = _is_da3_dataset(dataloader.dataset)
    
    loggers = {}
    dist_logger = Logger("dist_loss", "train", window_size=print_log_freq)
    loggers["dist_loss"] = dist_logger
    action_logger = Logger("action_loss", "train", window_size=print_log_freq)
    loggers["action_loss"] = action_logger
    total_loss_logger = Logger("total_loss", "train", window_size=print_log_freq)
    loggers["total_loss"] = total_loss_logger
    action_waypts_cos_sim_logger = Logger("action_waypts_cos_sim", "train", window_size=print_log_freq)
    loggers["action_waypts_cos_sim"] = action_waypts_cos_sim_logger
    multi_action_waypts_cos_sim_logger = Logger("multi_action_waypts_cos_sim", "train", window_size=print_log_freq)
    loggers["multi_action_waypts_cos_sim"] = multi_action_waypts_cos_sim_logger
    ...
    for i, data in enumerate(tqdm_iter):
        ...
        if is_da3_dataset and len(data) == 10:  # DA3 dataset returns 10 items (8 original + 2 original images)
            (
                obs_image,
                goal_image,
                action_label,
            ...
            viz_obs_image = original_obs_image if num_images_log > 0 else None
            viz_goal_image = original_goal_image if num_images_log > 0 else None
        else:
            # Standard dataset format (8 items)
            (
                obs_image,
                goal_image,
                action_label,
            ...
            if num_images_log > 0 and image_log_freq != 0:
        ...
        # Process images for model input (this is already done in DA3 dataset)
        if not is_da3_dataset:
            obs_images = torch.split(obs_image, 3, dim=1)
            obs_images = [transform(obs_image).to(device) for obs_image in obs_images]
            obs_image = torch.cat(obs_images, dim=1)
            goal_image = transform(goal_image).to(device)
        else:
            # DA3 images are already processed, just move to device
            obs_image = obs_image.to(device)
            goal_image = goal_image.to(device)

        # Handle DA3 model specific input format
        if _is_da3_model(model):
            # Pass navigation images by keyword so goal_image is not mistaken for extrinsics.
            model_outputs = _forward_da3_nav_model(model, obs_image, goal_image)
        else:
            # Standard GNM/ViNT model
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm_iter):
            if is_da3_dataset and len(data) == 10:
                (
                    obs_image,
                    goal_image,
                    action_label,
            ...
            if _is_da3_model(model):
                dist_pred, action_pred = _forward_da3_nav_model(model, obs_image, goal_image)
            else:
