...
# 获取实际的数据集名称列表（排除非数据集的键）
def get_actual_dataset_names():
    """获取实际的数据集名称，排除像 'action_stats' 这样的配置项"""
    all_keys = list(data_config.keys())
    # 排除已知的非数据集配置项
    non_dataset_keys = {'action_stats'}
    dataset_names = [key for key in all_keys if key not in non_dataset_keys]
    dataset_names.sort()
...
def visualize_traj_pred(
    ...
    # 使用过滤后的数据集名称列表
    dataset_names = get_actual_dataset_names()
    
    # 调试信息：打印 dataset_names 和实际的 dataset_idx
    # print(f"DEBUG: filtered dataset_names = {dataset_names}")
    # print(f"DEBUG: len(dataset_names) = {len(dataset_names)}")
    ...
    for i in range(min(batch_size, num_images_preds)):
        ...
        # 正确获取 dataset_name，处理可能的多维数组情况
        dataset_idx = int(np.asarray(dataset_indices[i]).flatten()[0])
        # print(f"DEBUG: dataset_idx[{i}] = {dataset_idx}")
        
        # 添加边界检查和更安全的处理
        if dataset_idx < 0 or dataset_idx >= len(dataset_names):
            # print(f"WARNING: dataset_idx {dataset_idx} is out of range [0, {len(dataset_names)-1}]")
            # 尝试从 eval_type 中提取数据集名称
            if '_' in eval_type:
                possible_dataset = eval_type.split('_')[0]
                if possible_dataset in data_config and possible_dataset != 'action_stats':
                    dataset_name = possible_dataset
                    # print(f"INFO: Using dataset name from eval_type: {dataset_name}")
                else:
                    # 使用第一个可用的数据集作为默认
                    dataset_name = dataset_names[0] if dataset_names else "go_stanford"
                    # print(f"INFO: Using default dataset name: {dataset_name}")
            else:
                dataset_name = dataset_names[0] if dataset_names else "go_stanford"
                # print(f"INFO: Using default dataset name: {dataset_name}")
        else:
        ...
        # 先获取 goal_pos，再进行格式标准化
        goal_pos = batch_goals[i]
        pred_waypoints = batch_pred_waypoints[i]
        label_waypoints = batch_label_waypoints[i]

        # 标准化 goal_pos 格式，确保是二维坐标
        goal_pos_normalized = np.asarray(goal_pos).flatten()
        if goal_pos_normalized.size < 2:
            # 如果只有一个值，假设是 x 坐标，y 坐标设为 0
            goal_pos_normalized = np.array([goal_pos_normalized[0] if goal_pos_normalized.size > 0 else 0.0, 0.0])
        else:
            # 取前两个值作为 x, y 坐标
            goal_pos_normalized = goal_pos_normalized[:2]
        ...
        if normalized:
            ...
            goal_pos_normalized *= data_config[dataset_name]["metric_waypoint_spacing"]
