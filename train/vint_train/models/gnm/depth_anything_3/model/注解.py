...
# [:, 5, :, :, :] -> B * 3 * 224 * 224  hist + obs + goal
    def forward(
        ...
        """
        # 1. 处理图片
        # image: B * 7 * 3 * 224 * 224 curr_obs: image[:, 5, :, :, :] -> B * 3 * 224 * 224
        ...
        ) # [len = 4] * [len = 2] * feat.shape
        # aux_feats -> B * 1 * 256 * 384
        # 
        ...
        # 2. depth = self.depth_patch_embed(output.depth) --> output.depth B * 256 * 384
        
        #分两种方式 3.1 3.2
        # 3.1. aux_feats.obs + depth --> swin_MLP --> out
        # 3.1 B * 7 * 3 * 224 * 224 用out替换第6张obs --> vint后续(transformer --> action_pred + distance_pred)
        
        # 3.2 B * 6 * 3 * 224 * 224 --> self.transformer --> out: B * 256 * 384
        # 3.2 out + depth --> swin_MLP --> out
        # 3.2 out + goal --> vint后续(action_pred + distance_pred)
