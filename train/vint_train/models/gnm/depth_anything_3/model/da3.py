...
# from dinov2.dinov2.hub import depth
...
from depth_anything_3.model.self_attention import MultiLayerDecoder
...
class DepthAnything3Net(nn.Module):
    ...
    def __init__(
        ...
        context_size: int = 5,
        embed_dim: int = 384,
        transformer_vint: bool = False,
        mha_num_attention_heads: Optional[int] = 2,
        mha_num_attention_layers: Optional[int] = 2,
        mha_ff_dim_factor: Optional[int] = 4,
        len_traj_pred: Optional[int] = 5,
        learn_angle: Optional[bool] = True,
        nav_fusion_mode: str | None = None,
    ):
        ...
        self.depth_patch_embed = PatchEmbed(
            img_size=(224, 224),    # 你的深度图输入尺寸 (H, W)，和配置一致
            patch_size=14,          # Patch Size，和配置一致
            in_chans=1,           # 深度图是单通道，固定
            embed_dim=embed_dim,
            norm_layer=None,      # = 原始卷积行为，无归一化
            flatten_embedding=True
        )               

        # Navigation prediction heads (GNM style)
        ...
        self.decoder2 = MultiLayerDecoder(
        ...
        self.obs_goal_fusion = nn.Sequential(
        ...
        self.siamese_mlp = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),
            nn.ReLU(),
            nn.Linear(embed_dim, embed_dim),
            nn.ReLU()
    ...
    def forward(
        ...
        export_feat_layers: list[int] | None = [5, 7, 9, 11],
        infer_gs: bool = False,
        use_ray_pose: bool = False,
        ref_view_strategy: str = "saddle_balanced",
        obs_img: torch.Tensor | None = None,
        goal_img: torch.Tensor | None = None,
        history_imgs: torch.Tensor | None = None,
    ) -> Dict[str, torch.Tensor]:
        """
        Forward pass through the network.

        Args:
            x: Input images (B, N, 3, H, W). If provided, other image args are ignored.
            extrinsics: Camera extrinsics (B, N, 4, 4)
            intrinsics: Camera intrinsics (B, N, 3, 3)
            export_feat_layers: List of layer indices to extract features from
            infer_gs: Whether to infer Gaussian Splatting parameters
            use_ray_pose: Whether to use ray-based pose estimation
            ref_view_strategy: Strategy for reference view selection
            obs_img: Current observation image (B, 3, H, W). Used if x is None.
            goal_img: Goal image (B, 3, H, W). Used if x is None.
            history_imgs: Historical images (B, 5, 3, H, W). Used if x is None.

        Returns:
            Dictionary containing predictions and auxiliary features
        """
        # Support separate input arguments by concatenating them into x
        if x is None:
            assert obs_img is not None and goal_img is not None and history_imgs is not None, \
                "If x is not provided, obs_img, goal_img, and history_imgs must be provided."
            # Concatenate: [history(5), obs(1), goal(1)] -> (B, 7, 3, H, W)
            x = torch.cat([
                history_imgs,
                obs_img.unsqueeze(1),
            ...
            # Navigation training may pass stacked RGB frames as (B, 3*N, H, W).
            # Convert them to DA3's expected multi-view shape before DINOv2.
            assert x.shape[1] % 3 == 0, f"Expected stacked RGB channels, got {x.shape[1]} channels"
            x = torch.stack(torch.split(x, 3, dim=1), dim=1)
            if extrinsics is not None and isinstance(extrinsics, torch.Tensor) and extrinsics.dim() == 4 and extrinsics.shape[1] == 3:
                x = torch.cat([x, extrinsics.unsqueeze(1)], dim=1)
                extrinsics = None
        if x.dim() == 4:
            # 如果输入是 4D (B, C, H, W)，人为增加一个序列维度 N=1
            # 变成 5D (B, 1, C, H, W)
            x = x.unsqueeze(1)
        
        B, N, C, H, W = x.shape
        # assert N == 7, f"Input must contain 7 images: 5 hist + 1 obs + 1 goal, got {N}"
        ...
        for feat_group in feats:
            ...
            for feat in feat_group:
                # Select the current observation frame for the depth head.
                feat_obs = feat[:, self.context_size : self.context_size + 1]
                processed_group.append(feat_obs)
            feats_obs.append(tuple(processed_group))
        feats_obs = tuple(feats_obs)

        obs_idx = self.context_size
        goal_idx = self.context_size + 1
        assert N >= goal_idx + 1, f"Expected at least {goal_idx + 1} frames, got {N}"

        with torch.autocast(device_type=x.device.type, enabled=False):
            output = self._process_depth_head(feats_obs, H, W)
            if use_ray_pose:
        ...
        if curr_obs_depth.dim() == 3:
            curr_obs_depth = curr_obs_depth.unsqueeze(1)
        if curr_obs_depth.shape[-2:] != (H, W):
            curr_obs_depth = F.interpolate(
                curr_obs_depth,
                size=(H, W),
                mode="bilinear",
                align_corners=False,
            )
        depth_embed = self.depth_patch_embed(curr_obs_depth)

        obs_visual = aux_feats[3][:, obs_idx, :, :]
        goal_visual = aux_feats[3][:, goal_idx, :, :]
        obs_goal_visual = self.obs_goal_fusion(
            torch.cat([obs_visual, goal_visual], dim=-1)
        )

        token_count = obs_visual.shape[1]
        if depth_embed.shape[1] != token_count:
            depth_embed = F.interpolate(
                depth_embed.transpose(1, 2),
                size=token_count,
                mode="linear",
                align_corners=False,
            ).transpose(1, 2)

        proj_obs_visual = self.siamese_mlp(obs_visual)
        proj_depth = self.siamese_mlp(depth_embed)
        siamese_fused = torch.cat([proj_obs_visual, proj_depth], dim=-1)
        siamese_fused = self.fusion_linear(siamese_fused)

        hist_embeddings = aux_feats[3][:, :obs_idx, :, :].mean(dim=2)
        fused_obs_embedding = siamese_fused.mean(dim=1, keepdim=True)
        obs_history_sequence = torch.cat(
            [hist_embeddings, fused_obs_embedding], dim=1
        )
        obs_goal_embedding = obs_goal_visual.mean(dim=1, keepdim=True)
        temporal_sequence = torch.cat(
            [obs_history_sequence, obs_goal_embedding], dim=1
        )
        final_repr = self.decoder2(temporal_sequence)
        ...
        action_xy = torch.cumsum(action_pred[:, :, :2], dim=1)
        if self.learn_angle:
            action_angle = F.normalize(action_pred[:, :, 2:], dim=-1)
            action_pred = torch.cat([action_xy, action_angle], dim=-1)
        else:
            action_pred = action_xy
