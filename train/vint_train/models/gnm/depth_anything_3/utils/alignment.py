...
def set_sky_regions_to_max_depth(
    ...
    Args:
        depth: Depth tensor
        depth_conf: Depth confidence tensor (can be None or other types)
        non_sky_mask: Non-sky region mask
    ...
    # Handle case where depth_conf is not a tensor (None, bool, etc.)
    if depth_conf is None or not isinstance(depth_conf, torch.Tensor):
        # Create a confidence tensor with same shape as depth, filled with 1.0
