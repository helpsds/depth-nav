...
# DA3 model import
try:
    from vint_train.models.gnm.depth_anything_3.model.da3 import DepthAnything3Net
except ImportError:
    print("Warning: DA3 model not found, DA3 deployment will not be available")
...
def load_model(
    ...
    if model_type == "gnm":
        ...
        model = DepthAnything3Net(
            ...
            transformer_vint=config.get("transformer_vint", True),
            mha_num_attention_heads=config.get("mha_num_attention_heads", 2),
    ...
    if model_type == "nomad":
        ...
        # DA3 models are saved differently - check if it's a full checkpoint or just state dict
